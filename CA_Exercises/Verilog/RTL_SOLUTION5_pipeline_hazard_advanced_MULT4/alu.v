...
module alu #(
   ...
   // alu naive mult support
   reg signed [DATA_W-1:0] mul_hi,mul_lo;
   reg overflow_mul;
   ...
   always@(*)begin
      ...
      {mul_hi, mul_lo} = alu_in_0 * alu_in_1;         // Mult Operation
   end
   ...
   // mul overflow
   always@(*)begin
      if((alu_ctrl==MUL_OP) && (mul_hi[DATA_W-1]!=alu_out[DATA_W-1]))begin
         overflow_mul = 1'b1;
      end else begin
         overflow_mul = mul_hi[DATA_W-1]?(&mul_hi!=1'b1):(|mul_hi!=1'b0);
      end
   end
...
//   assign overflow_sub = (alu_in_0[DATA_W-1] == alu_in_1[DATA_W-1] && sub_out[DATA_W-1] != alu_in_0[DATA_W-1]) ? 1 : 0;
   ...
   // mul overflow
   always@(*)begin
         overflow = overflow_add || overflow_mul || overflow_sub; 
   end 
